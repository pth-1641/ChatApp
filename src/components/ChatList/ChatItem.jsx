...
function ChatItem({ roomId }) {
  ...
  useEffect(() => {
    if (messages[0]?.uid === uid) {
      ...
      const memberSender = members?.find((mem) => mem.uid === messages[0]?.uid);
  ...
  return (
    ...
    >
      <a className='flex items-center gap-3'>
        ...
        >
          {chatType === 'friend' ? (
            <img src={friend.photoURL} alt='' className='w-full object-cover' />
          ) : chatAvatar ? (
            <img
              src={chatAvatar}
              alt=''
        ...
        <div className='flex-between flex-1 gap-3'>
          ...
          <div className='text-gray-600 dark:text-gray-400'>
            <time className='text-xs'>{time?.length && formatTime(time)}</time>
          </div>
        </div>
      </a>
    </li>
